...
class AplicacionCarretera extends Controller
{
    public function aplicacion_carretera(){
        return view('jetfilter.aplicacion_carretera.aplicacion_carretera');
    }
    ...
    public function nuevo(){
        ...
        return view('jetfilter.aplicacion_carretera.nuevo', compact('resultado_marca', 'resultado_vehiculo'));
    }

    public function store(Request $request){
        if( $request->has('aplicacion_carretera') ){
        ...
        if( count($codigo_filtro) > 0 ){
            ...
            return redirect()->route('jet-filter.catalogo.aplicacion_carretera.nuevo')->with('codigo_incorrecto', 'true');
        }
        if($tipo == 3){
            return redirect()->route('jet-filter.catalogo.aplicacion_carretera')->with('creado', $codigo);
        }
    ...
    public function show($id){
        ...
        return view('jetfilter.aplicacion_carretera.show', compact('aplicacion_seleccionada', 'marca_seleccionada', 'modelo_seleccionado', 'motor_seleccionado') );
    }

    public function delete($id, Request $request){
        if( $request->routeIs('*.aplicacion_carretera.*') ){
        ...
        if( $request->routeIs('*.aplicacion_carretera.*') ){
            ...
            $eliminado = $codigo;
            return redirect()->route("jet-filter.catalogo.aplicacion_carretera")->with('eliminado', $eliminado);
        }
    }

    public function edit($id){
        $aplicacion_carretera = Aplicacion::find($id);
        ...
        return view('jetfilter.aplicacion_carretera.edit', compact('aplicacion_carretera', 'resultado_marca', 'resultado_vehiculo', 'resultado_aplicacion') );
    }
    ...
    public function update($id, Request $request){
        if( $request->has('aplicacion_carretera') ){
            ...
            if( count($codigo_filtro) > 0 ){
                ...
                return redirect()->route('jet-filter.catalogo.aplicacion_carretera.edit', [ 'id' => $id ])->with('codigo_incorrecto', 'true');
            }
            if($tipo == 3){
                return redirect()->route('jet-filter.catalogo.aplicacion_carretera')->with('creado', $codigo);
            }
