...
class AplicacionComercial extends Controller
{
    public function aplicacion_comercial(){
        return view('jetfilter.aplicacion_comercial.aplicacion_comercial');
    }
    ...
    public function nuevo(){
        ...
        return view('jetfilter.aplicacion_comercial.nuevo', compact('resultado_marca', 'resultado_vehiculo'));
    }

    public function store(Request $request){
        if( $request->has('aplicacion_comercial') ){
        ...
        if( count($codigo_filtro) > 0 ){
            ...
            return redirect()->route('jet-filter.catalogo.aplicacion_comercial.nuevo')->with('codigo_incorrecto', 'true');
        }

        if($tipo == 2){
            return redirect()->route('jet-filter.catalogo.aplicacion_comercial')->with('creado', $codigo);
        }
    ...
    public function show($id){
        ...
        return view('jetfilter.aplicacion_comercial.show', compact('aplicacion_seleccionada', 'marca_seleccionada', 'modelo_seleccionado', 'motor_seleccionado') );
    }

    public function delete($id, Request $request){
        if( $request->routeIs('*.aplicacion_comercial.*') ){
        ...
        if( $request->routeIs('*.aplicacion_comercial*') ){
            ...
            $eliminado = $codigo;
            return redirect()->route("jet-filter.catalogo.aplicacion_comercial")->with('eliminado', $eliminado);
        }
    }

    public function edit($id){
        $aplicacion_comercial = Aplicacion::find($id);
        ...
        return view('jetfilter.aplicacion_comercial.edit', compact('aplicacion_comercial', 'resultado_marca', 'resultado_vehiculo', 'resultado_aplicacion') );
    }
    ...
    public function update($id, Request $request){
        if( $request->has('aplicacion_comercial') ){
            ...
            if( count($codigo_filtro) > 0 ){
                ...
                return redirect()->route('jet-filter.catalogo.aplicacion_comercial.edit', [ 'id' => $id ])->with('codigo_incorrecto', 'true');
            }
            if($tipo == 2){
                return redirect()->route('jet-filter.catalogo.aplicacion_comercial')->with('creado', $codigo);
            }
