...
namespace App\Http\Requests\JetFilter\AireIndustrial;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\Rule;

class AireIndustrialUpdateRequest extends FormRequest
{
    /**
    ...
    {
        $filtro_industrial = $this->route('filtro_industrial');
        $rules = [
            'codigo' => [
                'required',
                'string',
                'max:16',
                Rule::unique('filtro_codificacion', 'codigo')->ignore($filtro_industrial->id_filtro),
                Rule::unique('espec_aireautomotriz', 'codigo'),
                Rule::unique('espec_aireindustrial', 'codigo')->ignore($filtro_industrial),
