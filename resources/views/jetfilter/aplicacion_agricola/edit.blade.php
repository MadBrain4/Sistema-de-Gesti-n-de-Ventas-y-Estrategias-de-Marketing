<x-Arriba_JetFilter>
    <x-slot name="title">
        Nuevo Producto de Aplicación Agrícola
    </x-slot>
</x-Arriba_JetFilter>
...
<section class="about_tabla_espe">
    <section class="about-if_tabla_esp">
        <div class="tex_tablas">
            <p>Crear Producto de Aplicación Agrícola</p>
        </div>
        <div class="tex_tablas">
    ...
    <section class="es_tabla">
        <div class="tex_tablas">
            <form action="{{ route('jet-filter.catalogo.aplicacion_agricola.update', [ 'id' => $aplicacion_agricola->id ] ) }}" method="POST" class="form_login" id="editar-formulario">
                @method('PUT')
                <table class="tabla_edi">
                    ...
                    <tr>
                        <th>Código WEB: </th>
                        <td>  
                            <input class="edi_inp" type="text" value="{{ $aplicacion_agricola->codigo }}" id="codigo_web" name="codigo" id="codigo" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                    ...
                    <tr>
                        <th colspan="2" style="text-align: center;">Aplicación Agrícola</th>
                    </tr>
                    <tr>
                        <th>Marca: </th>
                        <td>
                            <select name="marca" class="select" id="marca">
                                @foreach( $resultado_marca as $resul )
                                    @if( $resul->id == $aplicacion_agricola->id_marca )
                        ...
                        <td>
                            <select name="vehiculo" id="vehiculo" class="select">
                                @foreach($resultado_vehiculo as $resul)
                                    @if( $resul->id == $aplicacion_agricola->id_vehiculo)
                        ...
                        <td>
                            <select name="aplicacion" class="select" id="marca">
                                @foreach($resultado_aplicacion as $apl)
                                        @if( $apl->aplicacion == $aplicacion_agricola->aplicacion )
                    ...
                    <tr>
                        <th>Detalle: </th>
                        <td>
                            <input class="edi_inp" type="text" value="{{ $aplicacion_agricola->detalle }}" name="detalle" id="detalle" />
                        </td>  
                    </tr>
                    <tr ><td class="b_td"><input type="submit" value="Editar" id="aplicacion_agricola" name="aplicacion_agricola" class="boton" /></td></tr>
                </table>
            </form>
        </div>
    </section>
