...
<section class="about_tabla_espe">
    ...
    <section class="es_tabla">
        <div class="tex_tablas">
            <form action="{{ route('jet-filter.catalogo.aplicacion_carretera.update', [ 'id' => $aplicacion_carretera->id ] ) }}" method="POST" class="form_login" id="editar-formulario">
                @method('PUT')
                <table class="tabla_edi">
                    <input type="hidden" value="{{ $aplicacion_carretera->id }}" name="id">
                    <tr><th colspan="2" style="text-align: center;">Filtro</th></tr>
                    <tr>
                        <th>Código WEB: </th>
                        <td>  
                            <input class="edi_inp" type="text" value="{{ $aplicacion_carretera->codigo }}" id="codigo_web" name="codigo" id="codigo" />
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                        ...
                        <td>
                            <select name="marca" class="select" id="marca">
                                @foreach( $resultado_marca as $resul )
                                    @if( $resul->id == $aplicacion_carretera->id_marca )
                        ...
                        <td>
                            <select name="vehiculo" id="vehiculo" class="select">
                                @foreach($resultado_vehiculo as $resul)
                                    @if( $resul->id == $aplicacion_carretera->id_vehiculo)
                        ...
                        <td>
                            <select name="aplicacion" class="select" id="marca">
                                @foreach($resultado_aplicacion as $apl)
                                        @if( $apl->aplicacion == $aplicacion_carretera->aplicacion )
                    ...
                    <tr>
                        <th>Detalle: </th>
                        <td>
                            <input class="edi_inp" type="text" value="{{ $aplicacion_carretera->detalle }}" name="detalle" id="detalle" />
                        </td>  
                    </tr>
                    <tr>
                        <td class="b_td">
                            <input type="submit" value="Editar" id="aplicacion_carretera" name="aplicacion_carretera" class="boton" />
                        </td>
                    </tr>
                </table>
            </form>
