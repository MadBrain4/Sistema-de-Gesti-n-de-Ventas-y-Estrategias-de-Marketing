...
@php 
    $estados = ['Azua', 'Baoruco', 'Barahona', 'Dajabon', 'Distrito Nacional', 'Duarte', 'Elias Pina', 'El Seibo', 'Espaillat', 'Hato Mayor', 'Independencia', 'La Altagracia', 'La Romana', 'La Vega', 'Maria Trinidad Sanchez', 'Monseñor Nouel', 'Monte Cristi', 'Monte Plata', 'Pedernales', 'Peravia', 'Puerto Plata', 'Salcedo', 'Samana', 'Sanchez Ramirez', 'San Cristobal', 'San Jose', 'San Juan', 'San Pedro', 'Santiago', 'Santiago Rodriguez', 'Santo Domingo', 'Valverde']
@endphp

<x-Arriba_JetFilter>
    <x-slot name="title">
        Editar Distribuidoras - Republica Dominicana
    </x-slot>
</x-Arriba_JetFilter>
...
<section class="about_tabla_espe">
    <section class="about-if_tabla_esp">
        <div class="tex_tablas">
            <p>Editar Distribuidoras - Republica Dominicana</p>
        </div>
        <div class="tex_tablas">
            <a href="{{ route('jet-filter.distribuidoras.dominicana') }}" class="boton_nuevo">Atras</a>
        </div>
    </section>
    ...
    <section class="es_tabla">
        <div class="tex_tablas">
            <form action="{{ route('jet-filter.distribuidoras.update', ['id' => $distribuidora->id]) }}" method="POST" class="form_login" enctype="multipart/form-data">
                <input type="hidden" value="Republica Dominicana" name="pais" required />
                <table class="tabla_edi">
                    ...
                    <tr>
                        <th>Estado: </th>
                        <td> 
                            <select id="estados_dominicana" name="estado" class="select" required>
