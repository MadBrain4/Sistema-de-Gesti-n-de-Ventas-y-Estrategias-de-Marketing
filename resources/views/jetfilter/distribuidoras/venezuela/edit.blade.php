...
@php 
    $estados = ['Amazonas', 'Anzoategui', 'Apure','Aragua', 'Barinas', 'Bolivar', 'Carabobo', 'Caracas', 'Cojedes', 'Delta Amacuro', 'Falcon', 'Guarico', 'Lara', 'Merida', 'Miranda', 'Monagas', 'Nueva Esparta', 'Portuguesa', 'Sucre', 'Tachira', 'Trujillo', 'Vargas', 'Yaracuy', 'Zulia']
@endphp

<x-Arriba_JetFilter>
    <x-slot name="title">
        Editar Distribuidoras - Venezuela
    </x-slot>
</x-Arriba_JetFilter>
...
<section class="about_tabla_espe">
    <section class="about-if_tabla_esp">
        <div class="tex_tablas">
            <p>Editar Distribuidoras - Venezuela</p>
        </div>
        <div class="tex_tablas">
            <a href="{{ route('jet-filter.distribuidoras.venezuela') }}" class="boton_nuevo">Atras</a>
        </div>
    </section>
    ...
    <section class="es_tabla">
        <div class="tex_tablas">
            <form action="{{ route('jet-filter.distribuidoras.update', ['id' => $distribuidora->id]) }}" method="POST" class="form_login" enctype="multipart/form-data">
                <input type="hidden" value="Venezuela" name="pais" required />
                <table class="tabla_edi">
                    ...
                    <tr>
                        <th>Estado: </th>
                        <td> 
                            <select id="estados_venezuela" name="estado" class="select" required>
