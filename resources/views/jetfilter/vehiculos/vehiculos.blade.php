...
@if ( session()->has('actualizado') )
    <script>
        var modelo = {!! json_encode(session('actualizado'), JSON_HEX_TAG) !!}
        Swal.fire({
            icon: 'success',
            title: `La información de ${modelo} se ha actualizado`,
            timer: 1750,
        })
    </script>
@endif
@if ( session()->has('eliminado') )
    <script>
        var modelo = {!! json_encode(session('eliminado'), JSON_HEX_TAG) !!}
        var motor = {!! json_encode(session('motor'), JSON_HEX_TAG) !!}
        Swal.fire({
            icon: 'success',
            title: `Se ha eliminado el vehículo Modelo: ${modelo} Motor: ${motor}`,
            timer: 1750,
        })
    </script>
@endif
@if ( session()->has('creado') )
    <script>
        var vehiculo = {!! json_encode(session('creado'), JSON_HEX_TAG) !!}
        Swal.fire({
            icon: 'success',
            title: `Se ha creado el vehículo ${vehiculo}`,
            timer: 1750,
        })
    </script>
    @php 
...
<section class="about_tabla_espe">
    <section class="about-if_tabla_esp">
        <div class="tex_tablas">
            <p>Vehículos - Aplicaciones</p>
        </div>
        <div class="tex_tablas">
            <a href="{{ route('jet-filter.catalogo.vehiculos.crear') }}" class="boton">Nuevo</a>
        </div>
    </section>
...
<script>
    ...
    function getData(pagina){
        ...
        fetch('/jetfilter/catalogo/vehiculos/cargar', {
            method: 'POST',
            body: formData,
        })
